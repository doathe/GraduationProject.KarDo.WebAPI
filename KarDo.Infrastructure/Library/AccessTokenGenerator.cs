...
namespace KarDo.Infrastructure.EFCore.Library
{
    public class AccessTokenGenerator
    {
        ...
        public TokenInfo GetToken()
        {
            ...
            if (_context.ApplicationUserTokens.Count(x => x.UserId == _applicationUser.Id) > 0)
            {
                ...
                //Expire olmuş ise var olan token'ı siler.
                if (userTokens.ExpireDate <= DateTime.Now)
                {
                    var stat = DeleteToken();
                }
            }
            //Yeni bir token oluşturulur ve veritabanına kaydedilir.
        ...
        public async Task<bool> DeleteToken()
        {
            ...
            try
            {
                ...
                if (_context.ApplicationUserTokens.Count(x => x.UserId == _applicationUser.Id) > 0)
                {
                    ApplicationUserToken userTokens = userTokens = _context.ApplicationUserTokens.FirstOrDefault(x => x.UserId == _applicationUser.Id);
